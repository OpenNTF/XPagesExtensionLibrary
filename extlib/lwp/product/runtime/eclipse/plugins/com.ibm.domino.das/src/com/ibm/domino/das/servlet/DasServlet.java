...
public class DasServlet extends AbstractRestServlet {
	...
	/**
	 * The variable name [RestWebServices] that could be set in either Internet
	 * Site document for a domain,
	 * or a Server document for a specific server
	 * This variable will return a list of services that are enabled or "" if no
	 * rest services are enabled.
	 * 
	 */
	...
	private static final int CORE_SERVICE_GKF = 427; // Defined by core SAAS
														// code
	...
	private static Pattern s_acceptLanguagePattern = Pattern
			.compile("(\\w{1,8})(?:\\-(\\w{1,8}))?(?:\\;q=(\\d(?:\\.\\d)?))?"); // $NON-NLS-1$

	/**
	 * Private class used to keep track of what services are enabled.
	 */
	protected class DasService {
	...
	/**
	 * Override the service to do the access control check before processing the
	 * REST request
	 */
	@Override
	public void doService(HttpServletRequest request, HttpServletResponse response) throws ServletException,
			IOException {
		// System.out.println("Starting DasServlet request processing");
		StatsContext.init();
		...
		try {
			...
			// Wrap the http request for X-HTTP-Method-Override header
			// manipulation
		...
		// System.out.println("Completed DasServlet request processing");
	...
	protected void initService(Application application, String serviceName, String servicePath, String serviceVersion,
			int gatekeeperFeatureNumber) {
		try {
			DasService service = new DasService(serviceName, servicePath, serviceVersion, gatekeeperFeatureNumber,
					application);
			s_services.put(servicePath, service);
	...
	private void initCoreService() {
		...
		initService(application, CORE_SERVICE_NAME, CORE_SERVICE_PATH, CORE_SERVICE_VERSION, CORE_SERVICE_GKF);
	}

	private void initDataService() {
		Application application = new DataService();
		initService(application, DATA_SERVICE_NAME, DATA_SERVICE_PATH, DATA_SERVICE_VERSION, DATA_SERVICE_GKF);
	}
			...
			for (final IConfigurationElement configElement : configElements) {
				try {
					...
					if (serviceName == null || servicePath == null) {
						DAS_LOGGER.getLogger().warning(
								StringUtil.format("DAS service {0} ignored. Both name and path must be defined.",
										serviceName)); // $NLW-DasServlet.DASservice0ignoredBothnameandpath-1$
						continue;
					}
					...
					DAS_LOGGER.getLogger().fine(
							StringUtil.format("Found a DAS service extension: {0} (/{1})", serviceName, servicePath)); // $NON-NLS-1$
					...
					if (!(object instanceof Application)) {
						...
						DAS_LOGGER.getLogger().warning(
								StringUtil.format("DAS service {0} ignored. Class is the wrong type.", serviceName)); // $NLW-DasServlet.DASservice0ignoredClassisthewrong-1$
						continue;
					}

					// This is a critical section. Things can go wrong inside
					// registerApplication
					// (e.g. NoClassDefFound). So catch all exceptions and log
					// them, but continue
					// to the next service.

					Application application = (Application) object;
					initService(application, serviceName, servicePath, (serviceVersion != null) ? serviceVersion
							: VERSION_ZERO, iGkf);
			...
			if (service != null) {
				...
				if (!enabled) {
					if (DAS_LOGGER.getLogger().isLoggable(Level.FINE)) {
						DAS_LOGGER.getLogger().fine(StringUtil.format("The {0} service is disabled on this server.", // $NON-NLS-1$
								service.getName()));
					}
				}
				...
				if (enabled && ScnContext.getCurrentInstance().isScn()) {
					...
					if (service.getGkf() != 0) {
						...
						enabled = provider.isFeatureEnabled(service.getGkf(), customerId, userId);
					...
					if (enabled && StringUtil.isNotEmpty(customerId)) {
						try {
							...
							// Do nothing. Assume the request is NOT for a self
							// service trial customer.
						}
					}
				}
					...
					synchronized (service) {
						try {
							...
							DAS_LOGGER
									.warn(e,
											StringUtil
													.format("Automatically disabling {0} service because there was an error registering its resources.", // $NLW-DasServlet.Automaticallydisablingaservicebec-1$
															service.getName()));
						}
					}
				}
	 ...
	 * 
	 * <p>
	 * This method enables all the services listed in the Internet Site document
	 * or server document.
	 */
	 ...
	 * @throws IOException
	 */
	private void handleError(HttpServletResponse response, Response.Status status, Throwable t)
	...
	private void parseLocales(List<Locale> locales, String headerValue) {
		// Portions of this method were stolen from XspCmdHttpServletRequest.
		// The
		...
		// preference for the languages specified by that range. The quality
