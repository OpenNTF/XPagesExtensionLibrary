...
public class DasServlet extends AbstractRestServlet {
	...
	public void doService(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		try {
			...
			// The average time is an approximation because we are not
			// synchronizing threads.
			// When multiple threads update the same stat at the same time, the
			// calculation could
			// be off, but it's not worth keeping a thread waiting for better
			// precision.
			...
			if (StringUtil.isNotEmpty(serviceName)) {
				...
				if (StringUtil.isNotEmpty(category)) {
					...
					if (requests != 0) {
						stats.setInteger(serviceName + "." + category + "." + STATS_AVG_TIME,
								(int) (requestTime / requests));
					}
				}
			}
		}
	...
	private void initCoreService() {
		...
		try {
			...
			DasService service = new DasService(CORE_SERVICE_NAME, CORE_SERVICE_PATH, CORE_SERVICE_VERSION,
					CORE_SERVICE_GKF, application);
	...
	private void initDataService() {
		...
		try {
			...
			// Add the service to our map
			DasService service = new DasService(DATA_SERVICE_NAME, DATA_SERVICE_PATH, DATA_SERVICE_VERSION, 0,
					application);
				...
				try {
					...
					try {
						...
						DasService service = new DasService(serviceName, servicePath,
								(serviceVersion != null) ? serviceVersion : VERSION_ZERO, iGkf, application);
