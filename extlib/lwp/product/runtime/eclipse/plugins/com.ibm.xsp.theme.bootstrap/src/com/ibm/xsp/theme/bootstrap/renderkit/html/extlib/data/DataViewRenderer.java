...
public class DataViewRenderer extends com.ibm.xsp.extlib.renderkit.html_extended.data.DataViewRenderer {
	...
	protected Object getProperty(int prop) {
		switch (prop) {
		case PROP_BLANKIMG:
			return Resources.get().BLANK_GIF;
		// note, for an Alt, there's a difference between the empty string and
		// null
		case PROP_BLANKIMGALT:
			return ""; //$NON-NLS-1$
		case PROP_ALTTEXTCLASS:
			return "lotusAltText"; // $NON-NLS-1$

		case PROP_HEADERCLASS:
			return "clearfix"; // $NON-NLS-1$
		case PROP_HEADERLEFTSTYLE:
			return null;
		case PROP_HEADERLEFTCLASS:
			return "pull-left"; // $NON-NLS-1$
		case PROP_HEADERRIGHTSTYLE:
			return null;
		case PROP_HEADERRIGHTCLASS:
			return "pull-right"; // $NON-NLS-1$
		...
		case PROP_TABLECLASS:
			return "clearfix table dataview"; // $NON-NLS-1$
		case PROP_TABLEROWEXTRA:
			return "lotusMeta lotusNowrap"; // $NON-NLS-1$

		case PROP_COLLAPSEICON:
			return Resources.get().BLANK_GIF;
		case PROP_COLLAPSEICONSTYLE:
			return "padding-right:8px"; // $NON-NLS-1$
		case PROP_COLLAPSEICONCLASS:
			return Resources.get().getIconClass("minus-sign"); // $NON-NLS-1$
		case PROP_EXPANDICON:
			return Resources.get().BLANK_GIF;
		case PROP_EXPANDICONSTYLE:
			return "padding-right:8px"; // $NON-NLS-1$
		case PROP_EXPANDICONCLASS:
			return Resources.get().getIconClass("plus-sign"); // $NON-NLS-1$

		case PROP_TABLEROWINDENTPX:
			return 20;
		case PROP_TABLEFIRSTCELLCLASS:
			return "xspFirstCell"; // $NON-NLS-1$
		case PROP_SHOWICONDETAILSCLASS:
			return Resources.get().getIconClass("chevron-down"); // $NON-NLS-1$
		case PROP_HIDEICONDETAILSCLASS:
			return Resources.get().getIconClass("chevron-up"); // $NON-NLS-1$

		case PROP_UNREADICONCLASS:
			return Resources.get().getIconClass("file") + " xspUnreadIcon"; // $NON-NLS-1$
			// $NON-NLS-2$
		case PROP_READICONCLASS:
			return Resources.get().getIconClass("file") + " xspReadIcon"; // $NON-NLS-1$
			// $NON-NLS-2$

		case PROP_TABLEHDRCOLIMAGE_SORTBOTH_ASCENDING:
			return Resources.get().getIconClass("sort-by-attributes"); // $NON-NLS-1$
		case PROP_TABLEHDRCOLIMAGE_SORTBOTH_DESCENDING:
			return Resources.get().getIconClass("sort-by-attributes-alt"); // $NON-NLS-1$
		case PROP_TABLEHDRCOLIMAGE_SORTBOTH:
			return Resources.get().getIconClass("sort"); // $NON-NLS-1$
		case PROP_TABLEHDRCOLIMAGE_SORTED_ASCENDING:
			return Resources.get().getIconClass("sort-by-attributes"); // $NON-NLS-1$
		case PROP_TABLEHDRCOLIMAGE_SORTED_DESCENDING:
			return Resources.get().getIconClass("sort-by-attributes-alt"); // $NON-NLS-1$

			// the bootstrap sort header icons are 16x13 px
		case PROP_TABLEHDRCOLIMAGE_SORT_WIDTH:
			return "16"; //$NON-NLS-1$
		case PROP_TABLEHDRCOLIMAGE_SORT_HEIGHT:
			return "13"; //$NON-NLS-1$

			// adjust the empty icon style for better indentation
		case PROP_EMPTYICONSTYLE:
			return "width:20px;height:13px;"; // $NON-NLS-1$

		case PROP_SUMMARYTITLECLASS:
			return "xspDataViewSummary"; // $NON-NLS-1$
		}
	...
	protected void writeShowHideDetailContent(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
		...
		final boolean detailsOnClient = viewDef.detailsOnClient;
		final String linkId = c.getClientId(context) + (viewDef.rowDetailVisible ? HIDE_DELIMITER : SHOW_DELIMITER) + viewDef.rowPosition;
		...
		final String label = (String) getProperty(viewDef.rowDetailVisible ? PROP_HIDEICONDETAILSTOOLTIP : PROP_SHOWICONDETAILSTOOLTIP);
		...
		final String clazz = (String) getProperty(viewDef.rowDetailVisible ? PROP_HIDEICONDETAILSCLASS : PROP_SHOWICONDETAILSCLASS);

		w.startElement("span", c); // $NON-NLS-1$
		w.writeAttribute("class", clazz, null); // $NON-NLS-1$

		final String spanId = c.getClientId(context) + "_shChevron"; // $NON-NLS-1$
		w.writeAttribute("id", spanId, null); // $NON-NLS-1$

		// Defect 195925 - replace unnecessary title attribute with sr-only div
		// containing text
		...
		if (!detailsOnClient) {
			if (viewDef.viewRowRefresh) {
				final String refreshId = c.getClientId(context) + NamingContainer.SEPARATOR_CHAR + UIDataView.ROW_ID;
	...
	protected void writeExpandCollapseIcon(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
		final boolean leaf = isRowLeaf(context, c, viewDef);
		if (leaf) {
			final String icon = (String) getProperty(PROP_EMPTYICON);
			if (icon != null) {
				w.startElement("img", c); // $NON-NLS-1$
				w.writeAttribute("src", HtmlRendererUtil.getImageURL(context, icon), null); // $NON-NLS-1$
				final String iconAlt = (String) getProperty(PROP_EMPTYICONALT);
				if (ExtLibRenderUtil.isAltPresent(iconAlt)) {
					// "" - present but empty
					w.writeAttribute("alt", iconAlt, null); //$NON-NLS-1$
				}
				final String style = (String) getProperty(PROP_EMPTYICONSTYLE);
				if (StringUtil.isNotEmpty(style)) {
					w.writeAttribute("style", style, null); // $NON-NLS-1$
				}
				final String clazz = (String) getProperty(PROP_EMPTYICONCLASS);
			...
			final String categoryId = c.getClientId(context) + "__rowCat:" + viewDef.rowPosition; // $NON-NLS-1$
			w.writeAttribute("id", categoryId, null); // $NON-NLS-1$

			final boolean expanded = isRowExpanded(context, c, viewDef);
			final String icon = (String) getProperty(expanded ? PROP_COLLAPSEICON : PROP_EXPANDICON);
			if (icon != null) {
				final String linkId = c.getClientId(context) + (expanded ? SHRINK_DELIMITER : EXPAND_DELIMITER) + viewDef.rowPosition;
				w.startElement("a", c);
				w.writeAttribute("id", linkId, null); // $NON-NLS-1$
				w.writeAttribute("href", "javascript:;", null); // $NON-NLS-1$
				// $NON-NLS-2$
				// LHEY97CCSZ adding the role=button
				w.writeAttribute("role", "button", null); // $NON-NLS-1$ //
				// $NON-NLS-2$
				final String iconAlt = (String) getProperty(expanded ? PROP_COLLAPSEICONALT : PROP_EXPANDICONALT);
				...
				w.startElement("span", c); // $NON-NLS-1$
				w.writeAttribute("title", iconAlt, null); //$NON-NLS-1$
				final String style = (String) getProperty(expanded ? PROP_COLLAPSEICONSTYLE : PROP_EXPANDICONSTYLE);
				if (StringUtil.isNotEmpty(style)) {
					w.writeAttribute("style", style, null); // $NON-NLS-1$
				}
				final String clazz = (String) getProperty(expanded ? PROP_COLLAPSEICONCLASS : PROP_EXPANDICONCLASS);
				if (StringUtil.isNotEmpty(clazz)) {
					w.writeAttribute("class", clazz, null); // $NON-NLS-1$
				}
				// Defect 195918
	...
	protected void writeIconColumn(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
		...
		if (viewDef.iconColumn != null) {
			final String colStyle = viewDef.iconColumn.getStyle();
		...
		if (viewDef.iconFacet != null) {
			...
			final IconEntry entry = findIcon(context, w, c, viewDef);
			if (entry != null) {
				final String src = entry.getUrl();
				if (StringUtil.isNotEmpty(src)) {
					final boolean unread = src.contains("xpPostUnread"); // $NON-NLS-1$
					w.startElement("div", c); // $NON-NLS-1$

					final String alt = entry.getAlt();
					...
					final String iconStyle = entry.getStyle();
					if (StringUtil.isNotEmpty(iconStyle)) {
						w.writeAttribute("style", iconStyle, null); // $NON-NLS-1$
					}
					final String iconClass = (String) getProperty(unread ? PROP_UNREADICONCLASS : PROP_READICONCLASS);
					...
					w.writeAttribute("aria-hidden", "true", null); // $NON-NLS-1$
					// $NON-NLS-2$
	...
	protected void writeDataColumn(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
		...
		if (null != viewDef.summaryColumn) {
			final String colTitle = viewDef.summaryColumn.getColumnTitle();
			if (StringUtil.isNotEmpty(colTitle)) {
				final String thId = c.getNonChildClientId(context) + "_th_" + colTitle; //$NON-NLS-1$
		...
		final int width = 100 / viewDef.multiColumnCount;
		w.writeAttribute("style", "width: " + width + "%", null); // $NON-NLS-1$
		// $NON-NLS-2$
		if (!viewDef.hasCheckBoxColumn && !viewDef.hasIconColumn) {
			final String clazz = (String) getProperty(PROP_TABLEFIRSTCELLCLASS);
			if (StringUtil.isNotEmpty(clazz)) {
				w.writeAttribute("class", clazz, null); // $NON-NLS-1$
		...
		// Enclosing divsfcol
		w.startElement("div", c); // $NON-NLS-1$
		final int level = getColumnIndentLevel(context, c, viewDef);
		final int catLevel = Math.max(0, viewDef.categoryCount - 1);
		final String indentStyle = getIndentStyle(context, c, viewDef, level + catLevel);
		...
		if (viewDef.collapsibleRows) {
			if (null != viewDef.summaryColumn) {
				final String summaryStyle = viewDef.summaryColumn.getStyle();
				final String style = ExtLibUtil.concatStyles("padding-top:0px;", summaryStyle); // $NON-NLS-1$
	...
	protected void writeColumnHeader(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef, ValueColumn vc, int colIndex) throws IOException {
		...
		w.writeAttribute("role", "columnheader", null); // $NON-NLS-1$
		// $NON-NLS-2$

		final String style = ExtLibUtil.concatStyles((String) getProperty(PROP_TABLEHDRCOLSTYLE), vc != null ? vc.getHeaderStyle() : null);
		if (StringUtil.isNotEmpty(style)) {
			w.writeAttribute("style", style, null); // $NON-NLS-1$
		}
		final String clazz = ExtLibUtil.concatStyleClasses((String) getProperty(colIndex == 0 ? PROP_TABLEHDRFIRSTCOLCLASS : PROP_TABLEHDRCOLCLASS),
				vc != null ? vc.getHeaderStyleClass() : null);
		if (StringUtil.isNotEmpty(clazz)) {
			w.writeAttribute("class", clazz, null); // $NON-NLS-1$
		}

		if (vc != null) {
			final String colTitle = vc.getColumnTitle();
			if (StringUtil.isNotEmpty(colTitle)) {
				final String colName = vc.getColumnName();

				// Kind of hard coded here, which is not clean...
				final boolean dominoStyleIcons = viewDef.dataModel instanceof DominoViewDataModel;
				boolean sortable = false;
				if (StringUtil.isNotEmpty(colTitle)) {
					// Add id to the th element
					final String thId = c.getClientId(context) + "_th_" + colTitle; //$NON-NLS-1$
				...
				if (sortable) {
					...
					final int sortState = getColumnSortState(context, c, viewDef, colName);

					if (sortState == TabularDataModel.RESORT_ASCENDING) {
						w.writeAttribute("aria-sort", "ascending", null); // $NON-NLS-1$
						// $NON-NLS-2$
					} else if (sortState == TabularDataModel.RESORT_DESCENDING) {
						w.writeAttribute("aria-sort", "descending", null); // $NON-NLS-1$
						// $NON-NLS-2$
					} else if (sortState == TabularDataModel.RESORT_BOTH) {
						w.writeAttribute("aria-sort", "other", null); // $NON-NLS-1$
						// $NON-NLS-2$
					} else {
						w.writeAttribute("aria-sort", "none", null); // $NON-NLS-1$
						// $NON-NLS-2$
					}

					w.startElement("span", c); // $NON-NLS-1$

					final String linkId = c.getClientId(context) + SORT_DELIMITER + colName;
					w.startElement("a", c);
					w.writeAttribute("id", linkId, null); // $NON-NLS-1$
					...
					final int dominoSortIconCode = getDominoSortIconCode(context, c, viewDef, colName);
					w.writeAttribute("title", headerTitle, null); // $NON-NLS-1$
					final String iconTextDescription = getSortIconDescription(dominoSortIconCode);
					w.writeAttribute("aria-label", colName + ", " + iconTextDescription, null); // $NON-NLS-1$

					if (!dominoStyleIcons) {
						if (sortState == TabularDataModel.RESORT_ASCENDING) {
							final String clazz2 = (String) getProperty(PROP_TABLEHDRCOLLKASCCLASS);
							if (StringUtil.isNotEmpty(clazz2)) {
								w.writeAttribute("class", clazz2, null); // $NON-NLS-1$
							}
						} else if (sortState == TabularDataModel.RESORT_DESCENDING) {
							final String clazz2 = (String) getProperty(PROP_TABLEHDRCOLLKDESCLASS);
							if (StringUtil.isNotEmpty(clazz2)) {
								w.writeAttribute("class", clazz2, null); // $NON-NLS-1$
							}
						}
				...
				final boolean multiColumn = viewDef.multiColumnCount > 1;
				...
				if (dominoStyleIcons) {
					final int dominoSortIconCode = getDominoSortIconCode(context, c, viewDef, colName);
					final String sort = getDominoSortIcon(context, c, viewDef, dominoSortIconCode);
	...
	protected void writeCheckboxAccessibilityAttributes(FacesContext context, ResponseWriter w, String checkBoxId) throws IOException {
		...
		w.writeAttribute("aria-checked", "false", null);// $NON-NLS-1$ //
		// $NON-NLS-2$
	...
	protected void writeExtraColumn(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef, ExtraColumn col, int colIdx) throws IOException {
		w.startElement("td", c); // $NON-NLS-1$
		final String value = formatColumnValue(context, c, viewDef, col);
		if (!StringUtil.isEmpty(value)) {
			w.writeAttribute("role", "gridcell", null); // $NON-NLS-1$
			// $NON-NLS-2$

			// Add aria-describedby property that references the header id
			final String colTitle = col.getColumnTitle();
			if (StringUtil.isNotEmpty(colTitle)) {
				final String thId = c.getNonChildClientId(context) + "_th_" + colTitle; //$NON-NLS-1$
				if (StringUtil.isNotEmpty(thId)) {
					w.writeAttribute("aria-describedby", thId, null); // $NON-NLS-1$
		...
		final UIComponent facet = getExtraFacet(c, colIdx);
		if (facet != null) {
			...
			final String style = col.getStyle();
			if (StringUtil.isNotEmpty(style)) {
				w.writeAttribute("style", style, null); // $NON-NLS-1$
			}
	...
	protected void writeShowHide(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
		// Disable superclass client show/hide JS, as bootstrap version
		// implemented below
		// in writeDetailsOnClientJS and in xsp.mixin.js. But keep the hidden
		// field
	...
	protected void writeDetailsOnClientJS(FacesContext context, ResponseWriter w, AbstractDataView c, ViewDefinition viewDef) throws IOException {
		// TODO re-work the hide/show detail on client JS in super classes to
		// make this
		...
		final String rowId = c.getClientId(context);
		final String dataViewID = rowId.substring(0, rowId.lastIndexOf(":" + viewDef.dataModel.getRowIndex()));
		...
		final StringBuilder onkeydown = new StringBuilder();
