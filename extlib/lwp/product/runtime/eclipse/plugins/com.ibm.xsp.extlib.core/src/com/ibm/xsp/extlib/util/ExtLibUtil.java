...
public class ExtLibUtil {
	...
	static {
		if(isXPages900()) {
			// Prepare for 900
			try {
				final Class<?> c = Class.forName("com.ibm.xsp.util.XspUtil");//$NON-NLS-1$
	...
	public static final boolean isXPages852() {
		if(_852==null) {
			final Version v = Version.CurrentRuntimeVersion;
	...
	public static String getExtLibVersion() {
		try {
			final String s = AccessController.doPrivileged(new PrivilegedAction<String>() {
				@Override
				public String run() {
					final Object o = ExtLibCoreActivator.instance.getBundle().getHeaders().get("Bundle-Version"); // $NON-NLS-1$
					if(o!=null) {
						return o.toString();
					}
					return null;
	...
	public static String getXspProperty(String name) {
		final XSPContext context = getXspContext();
		if(context!=null) {
	...
	public static Database getCurrentDatabase() {
		...
		final NotesContext nc = NotesContext.getCurrentUnchecked();
	...
	public static String getStringProperty(XSPContext ctx, String propName, String defaultValue) {
		final String v = ctx.getProperty(propName);
		if(v==null) {
			return defaultValue;
		}
		return v;
	...
	public static String getString(Map<String,Object> map, String propName, String defaultValue) {
		final Object v = map!=null ? map.get(propName) : null;
	...
	public static Map<String,Object> getMap(Map<String,Object> map, String propName) {
		if(map!=null) {
			final Map<String,Object> v = (Map<String,Object>)map.get(propName);
			return v;
		}
		return null;
	}
	...
	public static String getPageLabel(String pageName) {
		if(StringUtil.isNotEmpty(pageName)) {
			final int pos = pageName.lastIndexOf('/');
			if(pos>=0) {
	...
	public static String getClientId(FacesContext context, UIComponent start, String id, boolean forRefresh) {
		if(StringUtil.isNotEmpty(id)) {
			...
			final UIComponent c = FacesUtil.getComponentFor(start, id);
			if(c!=null) {
	...
	public static String getPartialRefreshUrl(FacesContext context, UIComponent component) {
		final ExternalContext ctx = context.getExternalContext();
		final String contextPath = ctx.getRequestContextPath();
		final String servletPath = ctx.getRequestServletPath();
		...
		final String ajaxId = component.getClientId(context);
		b.append('?');
		...
		final String vid = UniqueViewIdManager.getUniqueViewId(context.getViewRoot());
	...
	public static FacesExceptionEx newException(String msg, Object... params) {
		final String text = StringUtil.format(msg,params);
	...
	public static void pushParameters(FacesContext context, Map<String,String> parameters) {
		...
		if(parameters!=null) { // TODO why? the request params are already in the requestScope.
			final Map<String, Object> req = TypedUtil.getRequestMap(context.getExternalContext());
	...
	public static String readParameter(FacesContext context, String name) {
		...
		final Object reqMapObj = context.getExternalContext().getRequestMap().get(name);
	...
	public static void assignBindingProperty(FacesContext context, String bindingExpr, UIComponent component) {
		final ValueBinding binding = ((ApplicationEx)context.getApplication()).createValueBinding(bindingExpr);
	...
	public static void addScript(FacesContext context, String script) {
		if(StringUtil.isNotEmpty(script)) {
			final UIScriptCollector sc = UIScriptCollector.find();
			sc.addScript(script);
		}
	}
	...
	public static void addPendingScript(FacesContext context, String script) {
		...
		if(AjaxUtil.isAjaxPartialRefresh(context)) {
			final boolean isRendering = AjaxUtil.isRendering(context);
			AjaxUtil.setRendering(context, true);
			try {
				final ScriptResource r = new ScriptResource();
	...
	public static UIViewRootEx2 getViewRoot() {
		return (UIViewRootEx2) ExtLibUtil.resolveVariable("view");
	}
}