...
public class DojoAccordionPaneRenderer extends DojoContentPaneRenderer {
    
    @Override
    protected String getDefaultDojoType(FacesContext context, FacesDojoComponent component) {
        return "extlib.dijit.AccordionPane"; // $NON-NLS-1$
        //return "dijit.layout.AccordionPane"; // $NON-NLS-1$
    }
    
    @Override
    protected DojoModuleResource getDefaultDojoModule(FacesContext context, FacesDojoComponent component) {
        return ExtLibResources.extlibAccordionPane;
    }
    ...
    protected void initDojoAttributes(FacesContext context, FacesDojoComponent dojoComponent, Map<String,String> attrs) throws IOException {
        ...
        if(dojoComponent instanceof UIDojoAccordionPane) {
            UIDojoAccordionPane c = (UIDojoAccordionPane)dojoComponent;
            
            // Find if the pane is selected
            boolean selected = false;
            UIComponent p = c.getParent();
            if(p instanceof UIDojoAccordionContainer) {
                String sel = ((UIDojoAccordionContainer)p).getSelectedTab();
